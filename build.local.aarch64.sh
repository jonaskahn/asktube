...
(
    ...
    docker buildx build --platform linux/arm64 --tag ifelsedotone/asktube-web:latest . --load 2>&1 | tee "../$WEB_LOG"
) &  # Run in background
...
(
    ...
    docker buildx build --platform linux/arm64 --tag ifelsedotone/asktube-engine:latest . --load 2>&1 | tee "../$ENGINE_LOG"
) &  # Run in background
